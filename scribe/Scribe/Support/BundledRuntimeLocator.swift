...
struct BundledRuntimeLocator {
    ...
    static func userRuntimeDirectoryURL(bundle: Bundle = .main) -> URL {
        ...
        let appFolderName =
            (bundle.object(forInfoDictionaryKey: "CFBundleName") as? String) ?? "Scribe"
        return
            appSupportRoot
    ...
    /// read-only source, never a replacement: Jot's own writable directory is always scanned and
    ...
    /// Jot's own directory is always first (it is authoritative and the download target); the LM
    ...
    /// source additive: the picker shows Jot's own models plus the LM Studio library together.
    /// Candidate order is preserved (Jot's directory first) and duplicate filenames are deduped,
