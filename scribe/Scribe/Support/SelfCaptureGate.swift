...
/// File overview:
/// The pure decision behind Jot's "never complete inside our own UI" rule and its single
/// sanctioned exception. It is kept free of Accessibility objects and timer state so the invariant
/// that matters most here can be unit-tested directly: completions must never leak into Jot's own
...
enum SelfCaptureGate {
    ...
    ///
    /// Apps other than Cotabby are always allowed: this rule only constrains capturing Jot's own
